...
<body class="hold-transition skin-blue sidebar-mini">
									...
									<c:if test="${ not empty perduzenle }">
										<div class="col-xs-4">
											...
											<input type="hidden" name="perID"
												value='<c:out value="${ perduzenle.getPerID() }"></c:out>'>
											<div class="form-group">
												...
												<div class="input-group date">
													...
													<input type="text"
														...
														class="form-control pull-right" name="perDogumTarihi"
														id="datepicker" data-inputmask="'alias': 'dd/mm/yyyy'"
														data-mask="">
												</div>
										...
										<div class="col-xs-4">
											<div class="form-group">
												<label>Öğrenim Durumu</label> <select name="perOgrenimDurumu"
													class="form-control">

													<c:choose>
														<c:when
															test="${ perduzenle.getPerOgrenimDurumu()=='Ön Lisans' }">
															<option selected>Ön Lisans</option>
														</c:when>
														<c:otherwise>
													...
													<c:choose>
														<c:when test="${ perduzenle.getPerOgrenimDurumu()=='Lisans' }">
													...
													<c:choose>
														<c:when test="${ perduzenle.getPerOgrenimDurumu()=='Yüksek Lisans' }">
														...
														<c:choose>
														<c:when test="${ perduzenle.getPerOgrenimDurumu()=='Doktora' }">
															<option selected>Doktora</option>
														</c:when>
														<c:otherwise>
													...
													<div class="form-group">
														...
														<textarea name="perAdres" placeholder="Adres"
															class="form-control" rows="3"><c:out
																value="${ perduzenle.getPerAdres() }"></c:out></textarea>
													</div>
												</div>
												<div class="col-xs-4">
													<div class="form-group">
	...
	<script>
		$('#btnGucellePersonel').click(function() {
			var personel = $('#frmPersonel').serialize();
			//ajax
			$.ajax({
				url : '<s:url value="/personelguncellekaydet"></s:url>',
				type : "post",
				data : personel,
